...
> **NOTE:**
>
> Is this the right tutorial to start with? It picks up where [First data analysis with Python in a Jupyter Notebook](tutorial-get-started-ipynb.llms.md) and [Migrate from VS Code to Positron](tutorial-migrate-from-vscode.llms.md) leave off. If you have not worked through those, start there. This tutorial assumes you already know how to install Positron, create a Python environment, and run a notebook cell.

Positron is a free, source-available IDE for data science, built on the same open source core as VS Code. In the earlier tutorials you built a notebook by hand. In this one you keep your hands on the same tools. You also bring an AI assistant alongside you to write, extend, and reshape the analysis, while you stay in control of the result.
...
- Ghost cell suggestions, which predict the next cell after you run one
- The `/notebook` and `/report` commands, which turn a conversation into a Jupyter notebook or a Quarto document
...
The example files live in a GitHub repository. Cloning it gives you the data and a `requirements.txt` that lists the packages the analysis needs.

You can clone a repository without leaving the editor. The **New Folder from Git** option, available on the Welcome screen or from the Command Palette, clones a remote repository and opens it as your project.

> **TIP:**
>
> Open the Command Palette with , run *Workspaces: New Folder from Git*, and paste the repository URL:
>
> ``` default
...
Before setting a goal, look at what you have. The dataset is an Excel file, `data/retail_sales.xlsx`. You can open Excel files directly in the Data Explorer and inspect them without writing any code or converting the file first.

The Data Explorer is a sortable, filterable grid with per-column summary statistics. It gives you four ways to get your bearings in an unfamiliar table:

- Scroll through the rows to see the grain of the data.
- Pin a column so it stays in view while you scroll sideways.
- Sort a column by clicking its header.
- Filter to a subset with the filter controls, without changing the underlying file.

> **TIP:**
>
> In the **Explorer**, open `data/retail_sales.xlsx`. It opens in the Data Explorer as a grid of retail orders.
>
> 1.  Scroll down to get a feel for the roughly 9,800 rows.
> 2.  Pin the `order_id` column so it stays visible.
> 3.  Sort by `sales` to find the largest single order.
> 4.  Open a column’s summary to see its distribution.
>
...
You can do this by hand in the Data Explorer, but a one-off exploration is hard to rerun and hard to share. To keep a reproducible record, you will express the work as Python code. That means the next step is to set Positron up to run Python for this project.

## Set up a reproducible Python environment

A virtual environment is a self-contained Python setup with its own packages. You will add one to this project so its dependencies stay separate from everything else on your computer. The `requirements.txt` in the workshop repository pins the exact versions the analysis needs, including the pandas, openpyxl, itables, and plotnine packages.
...
You are set up to run Python reproducibly. Now bring in the assistant that will help you write it.

Posit Assistant is the AI assistant built into Positron. It does not include a language model of its own. Instead, it sends your prompt, along with context such as your open files and your session’s data, to a language model provider that you choose and sign in to. The provider runs the model and returns the response, which Positron shows in the chat. Positron lists every provider by default, but none is active until you authenticate with it.

Because providers differ in what account and credentials they need, the steps below are deliberately general. Use whichever provider you already have access to. If you do not have one, [Posit AI](assistant-providers.llms.md#posit-ai) is the most direct way to get started. You create an account at [posit.ai](https://posit.ai/) and sign in through your browser, with no API keys to manage.
...
With a provider connected, describe the analysis in plain language and let Posit Assistant draft the code. A good request names the file, the packages, and the outputs you want, so the assistant has enough to work with.
...
If a result looks off, say so in the same conversation. Posit Assistant keeps the context and revises, so you can steer it toward the picture you predicted rather than starting over.

## Export the conversation as a notebook

The analysis works, but right now it lives in a chat. To keep it as a reproducible artifact, export the conversation as a Jupyter notebook with the `/notebook` command. It collects the code from your conversation into a `.ipynb` file that you can rerun, edit, and commit.
...
[![Notebook Editor action bar with the kernel selector dropdown highlighted, showing a Python virtual environment selected.](images/positron-notebook-editor-kernel-selector.png)](images/positron-notebook-editor-kernel-selector.png "The kernel selector in the Positron Notebook Editor action bar, showing the project’s virtual environment selected as the kernel.")

The kernel selector in the Positron Notebook Editor action bar, showing the project’s virtual environment selected as the kernel.

## Work with AI inside the notebook

The Positron Notebook Editor is notebook-aware: Posit Assistant can see your cells, their outputs, and the order you ran them. That context powers a set of AI features you can reach without leaving the notebook. For the best view of them, run *View: Notebook Layout* to arrange the assistant, the notebook, and the Variables pane side by side.

[![Positron IDE in Notebook layout: Posit Assistant chat pane on the left, Notebook Editor in the center showing code and a bar chart, and the Variables pane on the right.](images/positron-notebook-layout.png)](images/positron-notebook-layout.png "The Positron IDE in Notebook layout, with the assistant chat on the left, the notebook in the center, and the Variables pane on the right.")

The Positron IDE in Notebook layout, with the assistant chat on the left, the notebook in the center, and the Variables pane on the right.

The notebook offers four AI entry points:

- The **Notebook Assistant panel**, opened from the **Assistant** button in the action bar, shows which cells are in context and offers quick actions to explain, fix, or improve the notebook.
...
- **Follow Assistant** scrolls the notebook to each cell as Posit Assistant edits it, so you can watch changes land.

[![Notebook Assistant panel showing context cells, quick actions like Fix, Explain, and Improve, and AI-generated suggestions based on the notebook state.](images/positron-notebook-assistant-panel.png)](images/positron-notebook-assistant-panel.png "The Notebook Assistant panel, showing the cells in context, quick actions to fix, explain, or improve the notebook, and generated suggestions.")

The Notebook Assistant panel, showing the cells in context, quick actions to fix, explain, or improve the notebook, and generated suggestions.
...
Ghost cell suggestions predict your next step after you run a cell. A ghost cell appears below the current one with a suggested action you can accept, edit, or dismiss. The notebook proposes where to go next instead of waiting for a blank cell. Ghost cell suggestions are experimental and off by default, so you enable them first.

> **TIP:**
>
> 1.  Set [`positron.assistant.notebook.ghostCellSuggestions.enabled`](positron://settings/positron.assistant.notebook.ghostCellSuggestions.enabled) to `true`.
> 2.  Run the cell that builds your `quarterly` table.
> 3.  Wait for the ghost cell to appear below it. To trigger it yourself instead, disable [`positron.assistant.notebook.ghostCellSuggestions.automatic`](positron://settings/positron.assistant.notebook.ghostCellSuggestions.automatic) and press .
> 4.  Accept a suggestion that adds something useful, such as the quarter with the highest sales, or edit it before accepting.
>
...
A notebook is a great place to work. When you want to share results as a document, a Quarto report is a better container. It renders the same code and prose to HTML, PDF, Word, and more, so one source can become whatever format your audience needs.

The `/report` command creates a Quarto document from your conversation, the same way `/notebook` creates the Jupyter notebook.

> **TIP:**
>
> Return to your Posit Assistant conversation, type `/report`, and send it. Posit Assistant writes a Quarto (`.qmd`) document from the analysis.
>
> **You will know it worked when** a `.qmd` file opens containing your code and explanatory prose. Render it to preview the report.
...
> ```
>
> Then render the dashboard to preview it.
>
> **You will know it worked when** the report renders as a two-column dashboard with a control to switch between light and dark themes.
...
> **TIP:**
>
> Open the Git pane, commit your notebook, report, and dashboard, and push them to the workshop repository or a fork of your own.
>
> **You will know it worked when** your files appear in the remote repository’s history.

## Wrap up

You took one spreadsheet from raw data to a published dashboard, with AI alongside you the whole way. Along that path you:
...
- Turned the analysis into a Quarto report with `/report` and reshaped it into a publishable dashboard
