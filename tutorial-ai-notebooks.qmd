...
::: {.callout-note}
Is this the right tutorial to start with? It picks up where [First data analysis with Python in a Jupyter Notebook](tutorial-get-started-ipynb.qmd) and [Migrate from VS Code to Positron](tutorial-migrate-from-vscode.qmd) leave off. If you have not worked through those, start there. This tutorial assumes you already know how to install Positron, create a Python environment, and run a notebook cell.
:::
...
Open the Command Palette with {{< kbd mac=Command-Shift-P win=Ctrl-Shift-P linux=Ctrl-Shift-P >}}, run _Workspaces: New Folder from Git_, and paste the repository URL:
...
<!-- SME: short screen capture of New Folder from Git cloning the workshop repo and opening it -->
:::
...
<!-- SME: screenshot of retail_sales.xlsx open in the Data Explorer with a column pinned and sorted -->
:::
...
<!-- SME: screen capture of Python: Create Environment installing from requirements.txt -->
:::
...
Because providers differ in what account and credentials they need, the steps below are deliberately general. Use whichever provider you already have access to. If you do not have one, [Posit AI](assistant-providers.qmd#posit-ai) is the most direct way to get started. You create an account at [posit.ai](https://posit.ai/) and sign in through your browser, with no API keys to manage.

<!-- SME: confirm the current Posit AI free-trial / pricing wording to mention here for students who want a low-cost way to try it. -->
...
<!-- SME: screen capture of Posit Assistant generating and running the analysis, ending with the itables table and plotnine chart -->
:::
...
![The kernel selector in the Positron Notebook Editor action bar, showing the project's virtual environment selected as the kernel.](images/positron-notebook-editor-kernel-selector.png){fig-alt="Notebook Editor action bar with the kernel selector dropdown highlighted, showing a Python virtual environment selected."}
...
![The Positron IDE in Notebook layout, with the assistant chat on the left, the notebook in the center, and the Variables pane on the right.](images/positron-notebook-layout.png){fig-alt="Positron IDE in Notebook layout: Posit Assistant chat pane on the left, Notebook Editor in the center showing code and a bar chart, and the Variables pane on the right."}
...
![The Notebook Assistant panel, showing the cells in context, quick actions to fix, explain, or improve the notebook, and generated suggestions.](images/positron-notebook-assistant-panel.png){fig-alt="Notebook Assistant panel showing context cells, quick actions like Fix, Explain, and Improve, and AI-generated suggestions based on the notebook state."}
...
<!-- SME: reuse or update videos/notebook-fix-error.mp4 here if it still matches the current Fix/Explain UI -->
:::
...
3. Wait for the ghost cell to appear below it. To trigger it yourself instead, disable [`positron.assistant.notebook.ghostCellSuggestions.automatic`](positron://settings/positron.assistant.notebook.ghostCellSuggestions.automatic) and press {{< kbd mac=Command-Shift-G win=Ctrl-Shift-G linux=Ctrl-Shift-G >}}.
...
<!-- SME: add a video shortcode here (videos/...mp4): screen capture of a ghost cell appearing after running the quarterly aggregation and being accepted -->
:::
...
<!-- SME: screenshot of the rendered two-column dashboard with the light/dark toggle -->
:::
